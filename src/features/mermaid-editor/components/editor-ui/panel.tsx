...
export function EditorPanelShell({ className, ...props }: HTMLAttributes<HTMLElement>) {
  return <section data-slot="editor-panel" className={cn("editor-ui-panel grid min-h-0 overflow-hidden text-foreground", className)} {...props} />;
}
...
export function EditorPanelBody({ className, padded = true, ...props }: HTMLAttributes<HTMLDivElement> & { padded?: boolean }) {
  return <div data-slot="editor-panel-body" className={cn("min-h-0", padded && "editor-ui-panel-body", className)} {...props} />;
}

export function EditorPanelFooter({ className, ...props }: HTMLAttributes<HTMLElement>) {
  return <footer data-slot="editor-panel-footer" className={cn("editor-ui-panel-footer flex items-center justify-between gap-3", className)} data-floating-panel-drag-exclude {...props} />;
}
