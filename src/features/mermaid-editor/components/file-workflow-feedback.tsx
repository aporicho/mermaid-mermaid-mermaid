...
export function FileDropFeedbackBadge({ feedback }: { feedback: FileDropFeedback }) {
  ...
  return (
    <div
      className={cn(
        "editor-ui-surface type-interface-status pointer-events-none absolute -translate-x-1/2 -translate-y-1/2 px-3 py-2",
...
export function FileWorkflowErrorBanner({ error, onClose }: { error: FileWorkflowError; onClose: () => void }) {
  return (
    <EditorNotice
      ...
      actions={<EditorIconButton context="inline" tone="danger" label="关闭文件错误提示" onClick={onClose}><Xmark data-icon /></EditorIconButton>}
