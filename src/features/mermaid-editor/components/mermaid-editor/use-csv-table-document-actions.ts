...
import { TABLE_LOADING_NODE_HEIGHT, TABLE_LOADING_NODE_WIDTH } from "@/features/mermaid-editor/lib/node-geometry";
import type { ProjectFileEntry, ProjectWorkspace } from "@/features/mermaid-editor/lib/project-workspace";

export function useCsvTableDocumentActions({
  runtime,
  graph,
  viewport,
  canvasLiveState,
  ...
  function addProjectCsvFile(
    ...
    if (existing) {
      ...
      applyEditorCommand({
        type: "viewport.set",
        viewport: {
          x: width / 2 - (existing.x + TABLE_LOADING_NODE_WIDTH / 2) * viewport.scale,
          y: height / 2 - (existing.y + TABLE_LOADING_NODE_HEIGHT / 2) * viewport.scale,
          scale: viewport.scale
        },
        source
      });
      setStatus(`画布中已存在 ${file.name}，已定位到表格。`);
      setDialogOpen(false);
      return;
    }
    ...
    applyEditorCommand({
      ...
      point: {
        x: point.x - TABLE_LOADING_NODE_WIDTH / 2,
        y: point.y - TABLE_LOADING_NODE_HEIGHT / 2
      },
      label: csvTableDocumentLabel(file),
      action: csvTableDocumentActionForProjectFile(file),
      message: `已添加 CSV 表格 ${file.name}。`,
      source
    });
    setDialogOpen(false);
    setExistingCollision(null);
  ...
  async function createCsvTable(value: string) {
    const fileName = normalizeNewCsvTableFileName(value);
    if (!fileName) {
      setStatus("请输入不含路径分隔符的 CSV 文件名。");
      return;
    }
    if (!projectWorkspace) {
      setStatus("请先打开项目文件夹，再新建 CSV 表格。");
      return;
    }
    ...
    setExistingCollision(null);
    try {
      const result = await runtime.createProjectTextFile({
        rootPath: projectWorkspace.rootPath,
        fileName,
        kind: "csv",
        text: initialCsvTableDocumentSource()
      });
      ...
      addProjectCsvFile(file);
    } catch (error) {
      showFileWorkflowError(error, "新建 CSV 表格失败。");
    } finally {
      setCreating(false);
    }
  }
  ...
  return {
    ...
    dialogProps: dialogOpen ? {
      ...
      existingCollision,
      onClose: closeDialog,
      onSelect: addProjectCsvFile,
      onCreate: createCsvTable,
      onUseExistingCollision() {
        if (existingCollision) addProjectCsvFile(existingCollision);
      }
    } : undefined
  };
}
