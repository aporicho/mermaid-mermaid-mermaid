...
import type { MermaidDocument } from "@/features/mermaid-editor/lib/mermaid-document";
import { isFlowchartNodeShape, normalizeFlowchartShape } from "@/features/mermaid-editor/lib/flowchart-shapes";
import { nextCanvasNodeId, toSafeNodeId } from "@/features/mermaid-editor/lib/mermaid-graph";
import {
  invalidEdgeMermaidId,
  invalidEnum,
  invalidId,
  missing,
  patchDiagnostic
} from "@/features/mermaid-editor/lib/mermaid-patch/diagnostics";
import {
  DEFAULT_VIEWPORT,
...
export function applyPatchOperations(document: MermaidDocument, ops: PatchOperation[]) {
...
function applyPatchOperation(graph: MermaidGraph, op: PatchOperation, state: PatchState): PatchOperationResult {
  switch (op.type) {
    case "addNode":
...
function setGraphOp(graph: MermaidGraph, op: Extract<PatchOperation, { type: "setGraph" }>, state: PatchState) {
  if (op.direction && !VALID_DIRECTIONS.has(op.direction)) return invalidEnum("direction", op.direction);
