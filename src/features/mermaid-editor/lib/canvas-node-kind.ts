...
import { isTextDocumentNode } from "@/features/mermaid-editor/lib/text-document";
...
export type CanvasNodeKind = "table" | "markdown-document" | "html-document" | "text-document" | "link-card" | "image" | "standard";
...
export function resolveCanvasNodeKind(node: CanvasNode): CanvasNodeKind {
  ...
  if (isHtmlDocumentNode(node)) return "html-document";
  if (isTextDocumentNode(node)) return "text-document";
