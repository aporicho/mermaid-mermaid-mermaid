...
function isRemovedCanvasDocument(value: unknown, fileName?: string, filePath?: string) {
  return value === "canvas" || [filePath, fileName].some((candidate) => candidate?.toLowerCase().endsWith(".canvas.json"));
}
...
export function loadInitialState() {
  ...
  try {
    ...
    if (isRemovedCanvasDocument(stored.documentKind, stored.fileName, stored.fileRef?.path)) {
      throw new Error("Legacy Pixi canvas documents are no longer supported");
    }
