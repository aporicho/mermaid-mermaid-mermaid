...
const Switch = React.forwardRef<React.ElementRef<typeof SwitchPrimitive.Root>, React.ComponentPropsWithoutRef<typeof SwitchPrimitive.Root>>(
  ({ className, ...props }, ref) => (
    <SwitchPrimitive.Root ref={ref} className={cn("editor-ui-focus inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-[length:var(--ui-border-width)] border-border bg-muted transition-colors data-[state=checked]:border-primary data-[state=checked]:bg-primary disabled:cursor-not-allowed disabled:opacity-[var(--ui-disabled-opacity)]", className)} {...props}>
