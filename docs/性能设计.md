...
- Konva Stage 和 Pixi stage viewport 使用命令式 position/scale 更新。
...
- Mermaid 画布和渲染视图的 viewport 热路径必须复用 `useViewportScheduler`，先更新视觉，再延迟提交 React 状态或 `viewport.set` 命令；`.canvas.json` Pixi 画布使用同等语义的命令式 rAF render scheduler。
...
- Konva/Pixi 大图渲染必须先通过 render scope 控制绘制量；裁剪只能影响绘制列表，不能改变命中语义、提交结果或文档结构。
