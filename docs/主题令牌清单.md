...
Markdown 阅读、编辑和浮动窗口共享 `theme.markdown`，并编译为 `--markdown-*` CSS 变量。内容宽度属于应用阅读偏好，不属于主题 token。

| Token 组 | 含义 | 当前字段 |
| --- | --- | --- |
| `markdown.font` | 正文、标题和代码的独立字体族 | `markdown.font.familyBody` / `familyHeading` / `familyCode` |
| `markdown.body` | 正文颜色、字号、行高、字重、字距和段落间距 | `markdown.body.*` |
| `markdown.heading.h1` … `h6` | 每级标题的颜色、字号、行高、字重、字距和上下间距 | `markdown.heading.*` |
| `markdown.link` | 链接默认/悬停颜色和下划线 | `markdown.link.*` |
| `markdown.emphasis` | 强调文字颜色与粗体字重 | `markdown.emphasis.*` |
| `markdown.list` | 列表标记颜色、缩进、列表项和列表块间距 | `markdown.list.*` |
| `markdown.quote` | 引用文字、边线、背景、内外间距和圆角 | `markdown.quote.*` |
| `markdown.inlineCode` | 行内代码颜色、字号、行高、内边距和圆角 | `markdown.inlineCode.*` |
| `markdown.codeBlock` | 代码块颜色、字号、行高、内外间距和圆角 | `markdown.codeBlock.*` |
| `markdown.table` | 表格文字、边框、表头、交替行、单元格间距和圆角 | `markdown.table.*` |
| `markdown.divider` | 分隔线颜色、粗细和上下间距 | `markdown.divider.*` |
| `markdown.image` | 图片边框、圆角和上下间距 | `markdown.image.*` |

当前主题 schema 为 v5。v4 及更早的自定义主题在加载时会从其归一化后的 UI 配色和字体派生完整 Markdown 默认值。
...
- `EditorTheme` 已升级为 v5，覆盖颜色、字体、Markdown 排版、尺寸、圆角、线宽、虚线、图标、画布交互、终端、ANSI 16 色、分组、连线标签、网格和诊断 token。
...
- 主题设置页已按概览、Markdown、画布、终端和高级分页，开放 Markdown 三类字体、完整语义样式、应用颜色、终端、ANSI 16 色和核心尺寸字段；虚线数组、应用字体家族和诊断阈值仍保留为 schema 级可配置，暂不在 UI 中直接编辑。
