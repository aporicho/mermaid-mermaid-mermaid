...
这是一个本地 Mermaid 编辑器，把源码编辑、官方渲染预览和可编辑的无限画布放在同一个工作区里。项目使用 Vite、React、TypeScript、Tailwind CSS、shadcn 风格 UI 原语、Mermaid、React Konva 和 Tauri 构建。
...
- 桌面端支持项目文件夹浏览，递归展示 `.mmd` / `.mermaid` 文件，并可在多个 Mermaid 文件之间切换。
- 支持三种工作区视图：无限画布、Mermaid 渲染视图、源码视图。
...
浏览器构建是静态 Vite 应用，负责人工编辑、查看、导入和导出 Mermaid 文件。桌面端在此基础上增加真实文件读写、项目文件夹扫描、图片资源导入和实时智能体桥接。
...
左侧面板是项目文件浏览器。桌面端会围绕当前文件或用户选择的文件夹递归扫描 Mermaid 文件；面板本身保持简洁，不放筛选输入框。右侧面板承载属性、主题和诊断。两个侧栏都以覆盖层形式浮在工作区上，不改变画布坐标系。

三视图切换规则：

- flowchart 文件可在无限画布、渲染视图和源码视图之间循环。
- 非 flowchart Mermaid 文件跳过无限画布，只在渲染视图和源码视图之间切换。
- 源码视图是独立工作区视图，不只是侧边栏；源码编辑提交后会刷新图模型、渲染结果和诊断。
